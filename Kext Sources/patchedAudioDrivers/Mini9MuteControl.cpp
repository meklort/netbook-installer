...
void Mini9MuteControl::setMute(bool mute)
{
	...
	switch(_output)
	{
		case WIDGET_HEAPHONES:
			...
			runVerb(0, _output, 0x3B0, (_mute ? 0x80 : 0x00));		// Set volume level
			if(!_mute) 	runVerb(0, _output, HDA_SET_EAPD, 0x02);	// Set headphones to enabled
			if(!_mute)	runVerb(0, _output, HDA_PIN_CTRL, 0xC0);	// Set headphones to enabled
